...
- TLS verification failures, invalid configuration, and output-file failures are fatal rather than endlessly retried. The session checks `SSL_get_verify_result` after a failed TLS handshake so certificate-chain/hostname failures remain distinguishable from transient transport loss during TLS negotiation; only the latter is recoverable. Treating certificate-chain or hostname failure as fatal deliberately prioritizes verified identity over availability; retrying the same endpoint with full verification would not itself be insecure, but this baseline chooses a visible failed run over waiting through a potentially persistent trust failure.
